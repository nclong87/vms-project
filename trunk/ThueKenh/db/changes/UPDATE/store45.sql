...
p_content varchar2(200);
BEGIN
  select t0.MADIEMDAU,t0.MADIEMCUOI,t1.LOAIGIAOTIEP,t0.DUNGLUONG,t0.USERCREATE into p_madiemdau,p_madiemcuoi,p_loaigiaotiep,p_dungluong,p_usercreate FROM tuyenkenh t0 left join loaigiaotiep t1 on t0.GIAOTIEP_ID=t1.ID where t0.id = tuyenkenh_id_;
  p_content := p_madiemdau || '-' || p_madiemcuoi || '-' || p_loaigiaotiep || '-' || p_dungluong || '-' || p_usercreate;
...
BEGIN
  ...
  FOR rec in (select distinct phone_number from sms_user where CREATE_TIME >= SYSDATE - 1 and phone_number is not null) loop
      ...
      DBMS_OUTPUT.PUT_LINE(v_sms_content7);
...
p_content varchar2(200);
BEGIN
  select t1.MADIEMDAU,t1.MADIEMCUOI,t2.LOAIGIAOTIEP,t1.DUNGLUONG,t1.USERCREATE,t0.USERCREATE,t0.TUYENKENH_ID into p_madiemdau,p_madiemcuoi,p_loaigiaotiep,p_dungluong,p_usercreate,p_userinput,p_tuyenkenhid FROM SUCOKENH t0 left join tuyenkenh t1 on t0.TUYENKENH_ID = t1.ID left join loaigiaotiep t2 on t1.GIAOTIEP_ID=t2.ID where t0.id = suco_id_;
  p_content := p_madiemdau || '-' || p_madiemcuoi || '-' || p_loaigiaotiep || '-' || p_dungluong || '-' || p_usercreate || '-' || p_userinput;
