--------------------------------------------------------
--  DDL for Procedure PROC_UPDATE_TRAM_BTS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "THUEKENH"."PROC_UPDATE_TRAM_BTS" AS 
v_matram varchar2(50);
v_diachi varchar2(500);
v_diachi1 varchar(500);
i INTEGER := 0;
BEGIN
  for rec in (select t0.*,t1.MATRAM,t1.DIACHI from V_BTS_SITE t0 left join TRAM t1 on TRIM(t0.SITE_KEY) = t1.matram) loop
    v_matram := TRIM(rec.SITE_KEY);
    v_diachi := TRIM(rec.PRECINCT) || ', ' || TRIM(rec.DISTRICT) || ', ' || TRIM(rec.PROVINCE);
    if(rec.MATRAM IS NULL) then --insert new
      i:=SEQ_TRAM.nextval;
      insert into TRAM (ID,MATRAM,DIACHI,DELETED) values (i,v_matram,v_diachi,0);
    elsif (rec.DIACHI is null OR v_diachi != rec.DIACHI) then -- update when diff address
      update TRAM set DIACHI = v_diachi where MATRAM = rec.MATRAM;
    end if;
  end loop;
END PROC_UPDATE_TRAM_BTS;
